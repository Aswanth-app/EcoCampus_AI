/**
 * EcoCampus AI — Water Intelligence Engine Automated Verification Suite (ESM)
 */
...
async function runAllTests() {
  ...
  // ----------------------------------------------------------------------------
  // Test 8: Full End-to-End Pipeline Evaluation Structure
  // ----------------------------------------------------------------------------
  console.log("\n--- Scenario 8: End-to-End Pipeline Output Contract ---");
  const fullPipelineResult = evaluateWaterTelemetry({
    currentReading: spikeReading,
    recentWindow: [
      { ...spikeReading, timestamp: "2026-09-01T14:59:50Z", flow_rate_lpm: 2.1 },
      spikeReading,
    ...
    baselineHistory: [],
    locationType: "restroom",
    locationName: "Hostel Block A — Ground Floor Restroom",
  });

  assert(typeof fullPipelineResult.compositeRiskScore === "number", "Risk score is numeric");
  assert(fullPipelineResult.anomalies.length > 0, "Anomalies array populated");
  assert(fullPipelineResult.primaryRecommendation !== null, "Primary recommendation object present");
  assert(typeof fullPipelineResult.primaryReason === "string", "Primary reason is string");
  assert(typeof fullPipelineResult.avoidableVolumeTodayLiters === "number", "Avoidable volume calculated");
